class Lyric < ActiveRecord::Base
  ...
  has_many :english_verses, :class_name=>'LyricVerse', :foreign_key=>'lyricsongid', :order=>'number', :conditions=>'#{"#{aliased_table_name}." rescue ""}languageid = 1'
  has_many :romaji_verses, :class_name=>'LyricVerse', :foreign_key=>'lyricsongid', :order=>'number', :conditions=>'#{"#{aliased_table_name}." rescue ""}languageid = 2'
  has_many :japanese_verses, :class_name=>'LyricVerse', :foreign_key=>'lyricsongid', :order=>'number', :conditions=>'#{"#{aliased_table_name}." rescue ""}languageid = 3'
